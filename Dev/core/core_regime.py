...
logger = logging.getLogger("yjcrypto-regime")

REGIME_FILE = Path("/root/.yjcryptosignal-bot/market_regime.json")

_prev_regime: Optional[str] = None
_regime_change_flag: bool = False
...
def detect_regime(df_btc, df_alt=None) -> dict:
    ...
    global _prev_regime, _regime_change_flag
    new_regime = regime_data["regime"]
    if _prev_regime is not None and new_regime != _prev_regime:
        _regime_change_flag = True
        logger.info(f"🌊 Regime changed: {_prev_regime} → {new_regime}")
...
def has_regime_changed() -> bool:
    """Returns True if regime changed since last check, then resets flag."""
    global _regime_change_flag
    if _regime_change_flag:
        _regime_change_flag = False
        return True
    return False


def get_previous_regime() -> Optional[str]:
    """Returns the previous regime type, or None if no detection has run."""
    return _prev_regime
...
def _compute_adx_simple(high, low, close, period=14):
    ...
    di_plus = np.divide(100 * up_s, atr, out=np.zeros_like(atr), where=atr > 0)
    di_minus = np.divide(100 * down_s, atr, out=np.zeros_like(atr), where=atr > 0)

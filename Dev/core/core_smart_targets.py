...
logger = logging.getLogger("yjcrypto-smart-targets")
...
def enhance_signal_targets(signal: dict, df, regime: str = "NORMAL") -> dict:
    ...
    try:
        ...
        # 🎯 ATR-based Duration Calculation —atum واقعي للمدة المتوقعة
        # مدة = (TP1_distance / ATR_distance) * candle_hours
        try:
            candle_hours = 4  # 4h timeframe
            tp1_dist_pct = abs(signal["targets"][0] - entry) / entry * 100
            atr_dist_pct = atr_pct * 100
            if atr_dist_pct > 0:
                estimated_candles = tp1_dist_pct / atr_dist_pct
                estimated_hours = estimated_candles * candle_hours
                signal["estimated_duration_hours"] = round(max(1, min(168, estimated_hours)), 1)  # clamp 1h-1week
                signal["estimated_duration_candles"] = round(max(0.25, estimated_candles), 2)
            else:
                signal["estimated_duration_hours"] = 12.0
                signal["estimated_duration_candles"] = 3.0
        except Exception as e:
            logger.debug(f"Duration calc skipped: {e}")
            signal["estimated_duration_hours"] = 12.0
            signal["estimated_duration_candles"] = 3.0

...
PROGRESS_FILE = Path("/root/.yjcryptosignal-bot/universal_progress.json")
BROADCAST_CACHE_FILE = Path("/root/.yjcryptosignal-bot/universal_broadcast_cache.json")
SIGNAL_MSGS_FILE = Path("/root/.yjcryptosignal-bot/signal_messages.json")
TRADES_FILE = Path("/root/.yjcryptosignal-bot/trades.json")
TRADES_HISTORY_FILE = Path("/root/.yjcryptosignal-bot/trades_history.json")
SCANNER_LOCK = os.getenv("SCANNER_LOCK_FILE", "/tmp/yjcryptosignal-scanner.lock")
...
MAX_ACTIVE_TRADES = int(os.getenv("MAX_ACTIVE_TRADES", "9999"))  # من ENV — غير محدود افتراضياً
MAX_SL_PCT = float(os.getenv("MAX_SL_PCT", "10.0"))
...
def _can_broadcast(symbol: str, broadcast_cache: dict) -> tuple:
   ...
   try:
        ...
        logger.warning(f"Broadcast cache read failed for {symbol}: {e}")
...
def _broadcast_update(symbol: str, update_text: str):
   ...
   try:
       ...
       from bot.bot_handlers import send_msg, load_subscribers
       sent = 0
...
def _count_active_trades() -> int:
   ...
   try:
      ...
      logger.warning(f"Failed to count active trades: {e}")
   return 0
...
def _gather_all_exchange_coins(fetcher, min_volume: float = 500000):
   ...
   """
   from data.data_fetcher import EXCLUDED_SYMBOLS
...
def _broadcast_signal(result, ai_result, symbol):
   try:
       ...
       from report.report_telegram import format_signal_report_arabic_simple
       from bot.bot_handlers import broadcast
       try:
           from core.core_regime import get_cached_regime
       ...
       try:
           from trade.trade_tracker import add_trade
           ...
           if success:
               ...
               try:
                   from trade.trade_userlists import subscribe_to_trade
...
# ═══════════════════════════════════════
# 🌊 REGIME BACKTEST DISPATCH
# ═══════════════════════════════════════

def _run_regime_backtest_async(regime: str):
    """Run regime backtest in background thread — non-blocking."""
    try:
        from engine.engine_backtest import run_regime_backtest
        summary = run_regime_backtest(regime)
        if summary:
            logger.info(f"📊 Regime backtest: {json.dumps(summary, default=str)}")
    except Exception as e:
        logger.debug(f"Regime backtest failed: {e}")
...
def universal_scan_loop():
   ...
   if os.path.exists(SCANNER_LOCK):
       try:
           ...
           os.kill(old_pid, 0)
           logger.warning(f"⚠️ Scanner lock found (PID {old_pid}) — another scanner is running, exiting")
           return
           ...
           try:
               ...
               logger.debug(f"Scanner lock cleanup failed: {e}")
       except Exception as e:
           logger.warning(f"Scanner lock check failed: {e}")
   ...
   try:
       ...
       logger.error(f"Failed to write scanner lock: {e}")
   ...
   while True:
       ...
       # ⏭️ Stock token pre-filter — شيل الأسهم المرمّزة (R/Z) من القائمة كلها قبل أي تحليل
       # (الأسهم لها volume ضخم وتتصدّر، فلازم تنشال قبل MTF عشان العيّنة تكون كريبتو حقيقي)
       _before_stock = len(coin_list)
       coin_list = [(s, ex) for s, ex in coin_list if not _is_stock_token(s)]
       _stock_removed = _before_stock - len(coin_list)
       if _stock_removed:
           logger.info(f"  ⏭️ Stock pre-filter: شيل {_stock_removed} سهم مرمّز ({_before_stock} → {len(coin_list)})")
       total = len(coin_list)

       # 🔬 Multi-TF Pre-filter — احذف العملات الي مش في اتجاه شراء على 2+ فريم
       try:
           from engine.engine_multi_tf import scan_strength_matrix
           top_by_volume = coin_list[:50]
           symbols_for_mtf = [s for s, _ in top_by_volume]
           mtf_results = scan_strength_matrix(symbols_for_mtf, max_coins=50)
           # Filter: keep only BUY with alignment >= 2 (2/3 or 3/3 timeframes agree)
           strong_symbols = {r["symbol"] for r in mtf_results if r["overall"] == "BUY" and r["alignment"] >= 2}
           if strong_symbols:
               # احتفظ بالقوية من العيّنة + كل العملات خارج العيّنة (لسا ما اتفحصت)
               sampled = set(symbols_for_mtf)
               coin_list = [(s, ex) for s, ex in coin_list if s in strong_symbols or s not in sampled]
               total = len(coin_list)
               logger.info(f"🔬 MTF filter: عيّنة {len(symbols_for_mtf)} → {len(strong_symbols)} قوية | إجمالي للتحليل: {total}")
           else:
               logger.warning("⚠️ MTF filter: no strong symbols in sample, using all")
       except Exception as e:
           logger.debug(f"MTF pre-filter skipped: {e}")
       ...
       for i in range(start_idx, total):
           ...
           try:
               ...
               try:
                   from core.core_regime import get_cached_regime, has_regime_changed
                   regime_data = get_cached_regime() or {}
                   if has_regime_changed():
                       threading.Thread(
                           target=_run_regime_backtest_async,
                           args=(regime_data.get("regime", "UNKNOWN"),),
                           daemon=True,
                       ).start()
               except Exception:
                   regime_data = {}
               ...
               if ai_decision == "ENTER" and ai_direction != "SELL":
                   ...
                   if sl_dist >= MAX_SL_PCT:
                       logger.info(f"  ⏭️ {symbol}: SL {sl_dist:.2f}% ≥ {MAX_SL_PCT}% — مخاطرة عالية, رفض")
                       coins_analyzed += 1
                       _wait_remaining(coin_start)
                       continue

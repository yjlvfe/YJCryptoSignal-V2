...
logger = logging.getLogger("yjcrypto-tracker")

DATA_DIR = Path(os.environ.get("DATA_DIR", "/root/.yjcryptosignal-bot"))
TRADES_FILE = DATA_DIR / "trades.json"
HISTORY_FILE = DATA_DIR / "trades_history.json"
MAX_TRADES = 9999  # لا يوجد حدود على الصفقات النشطة
        ...
        try:
            ...
            if len(trades) == 0 and TRADES_FILE.exists():
                try:
                    ...
                    logger.warning(f"Backup preserve check failed: {e}")
            ...
            if TRADES_FILE.exists():
                try:
                    ...
                    logger.warning(f"Backup rename failed: {e}")
...
def _save_to_history(closed_trades: list):
    ...
    try:
        ...
        try:
            from learn.learn_adaptive import evaluate_closed_trade
        ...
        # 📊 Update Expectancy — لكل استراتيجية شاركت في الصفقة
        try:
            from engine.engine_weights import update_expectancy
            for entry in history[-len(closed_trades):] if closed_trades else []:
                update_expectancy(entry)
        except Exception as e:
            logger.debug(f"Expectancy update skipped: {e}")
    except Exception as e:
...
def generate_daily_report() -> str:
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Regime lookup skipped: {e}")

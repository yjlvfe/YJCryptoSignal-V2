...
logger = logging.getLogger("yjcrypto-health")
...
def main():
    ...
    # 1. Bot process
    import subprocess
    result = subprocess.run(["pgrep", "-f", "bot/bot_main.py"], capture_output=True, text=True)
    ...
    total_keys = sum(p['keys_available'] for p in providers.values())
    healthy = sum(1 for p in providers.values() if p['healthy'])
    has_keys = total_keys > 0 and healthy > 0
    check("AI keys", has_keys, f"{healthy}/{len(providers)} healthy, {total_keys} total keys" if has_keys else "MISSING")
    if not has_keys:
        issues += 1
    ...
    try:
        ...
        result = call_ai("Reply only: OK", "Health check", max_tokens=5)
    ...
    import py_compile
    compile_errors = 0
    valid_files = [f for f in py_files if f.exists() and f.is_file()]
    for f in valid_files:
        try:
            py_compile.compile(str(f), doraise=True)
        except py_compile.PyCompileError:
            compile_errors += 1
    check("Python compilation", compile_errors == 0, f"{compile_errors}/{len(valid_files)} files fail" if compile_errors else f"{len(valid_files)} files OK")

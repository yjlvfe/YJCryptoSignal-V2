...
logger = logging.getLogger("yjcrypto-fetcher")
...
# ═══════════════════════════════════════
# 🗄️ Optional Redis Cache Layer
# ═══════════════════════════════════════
class RedisCache:
    """Optional Redis cache for market data. Falls back to in-memory dict cache if Redis unavailable."""

    def __init__(self):
        self._enabled = os.getenv("USE_REDIS", "false").lower() == "true"
        self._host = os.getenv("REDIS_HOST", "localhost")
        self._port = int(os.getenv("REDIS_PORT", "6379"))
        self._ttl = int(os.getenv("REDIS_CACHE_TTL", "60"))
        self._local: dict = {}
        self._redis = None
        if self._enabled:
            try:
                ...
                self._redis = rd.Redis(
                    host=self._host, port=self._port, socket_connect_timeout=2
                )
                self._redis.ping()
            except Exception:
                self._redis = None
...
class MultiExchangeFetcher:
    ...
    def fetch_klines(self, symbol: str, interval: str = "1d", limit: int = 200) -> pd.DataFrame:
        ...
        try:
            ...
            raise
        try:
            self._cache.set(cache_key, result.to_dict(orient="records"))
        except Exception:
            pass
        return result

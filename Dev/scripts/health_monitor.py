...
_is_new_system = (_PROJECT_DIR.name == "YJCryptoSignal")

# Point to the project root (auto-detects new vs old system)
PROJECT_DIR = _PROJECT_DIR  # Should be /root/projects/YJCryptoSignal
DATA_DIR = Path(os.getenv("DATA_DIR", "/root/.yjcryptosignal-bot"))
LOG_FILE = Path(os.getenv("HEALTH_MONITOR_LOG", str(DATA_DIR / "logs" / "health_monitor.log")))
TRADES_FILE = Path(os.getenv("TRADES_FILE", str(DATA_DIR / "trades.json")))
OI_CACHE_FILE = Path(os.getenv("OI_CACHE_FILE", str(DATA_DIR / "oi_cache.json")))
FIX_HISTORY_FILE = Path(os.getenv("FIX_HISTORY_FILE", str(DATA_DIR / "fix_history.json")))
MONITOR_START_FILE = PROJECT_DIR / "monitor_start.txt"
SCRIPTS_DIR = PROJECT_DIR / "strategies"
STRATEGIES_DIR = PROJECT_DIR / "strategies"

# ═══════════════ TELEGRAM CONFIG ═══════════════
# PRIMARY: TELEGRAM_BOT_TOKEN (new standard). FALLBACK: BOT_TOKEN (legacy compat).
# Never hardcoded — always from environment.
BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN") or os.getenv("BOT_TOKEN") or ""
if not BOT_TOKEN:
    logger.warning("⚠️ TELEGRAM_BOT_TOKEN not set — health monitor will not send Telegram messages")
    API_BASE = ""
else:
    API_BASE = f"https://api.telegram.org/bot{BOT_TOKEN}"
# PRIMARY: TELEGRAM_CHAT_ID (new standard). FALLBACK: OWNER_ID (legacy compat).
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID") or os.getenv("OWNER_ID") or ""
if not TELEGRAM_CHAT_ID:
    logger.warning("⚠️ TELEGRAM_CHAT_ID not set — health monitor cannot send Telegram alerts")
try:
...
# ═══════════════ PHASE 5 FILES ═══════════════
POSITION_SIZING_FILE = PROJECT_DIR / "trade" / "trade_sizing.py"
PORTFOLIO_HEAT_FILE = PROJECT_DIR / "trade" / "trade_heat.py"
...
def send_telegram(text):
    """Send report via Telegram bot API. Fails gracefully if credentials missing."""
...
def find_bot_pids():
    """Find all bot bot_main.py processes (real project bot, not Hermes sandbox)"""
    out, code = sh("pgrep -af 'bot/bot_main.py' 2>/dev/null")
...
def check_runtime_heartbeat():
    ...
    # ─── Telegram check ───
    if not API_BASE:
        signals["telegram_ok"] = False
        errors.append("❤️ RUNTIME: Telegram not configured (TELEGRAM_BOT_TOKEN missing)")
    else:
        try:
...
def check_1_bot_running():
    ...
    if not pids:
        ...
        repairs.append(("bot/bot_main.py", f"Killed {len(pids_sorted)-1} old duplicates", f"Keeping PID {pids_sorted[-1]}", True))
...
def check_8_phase5():
    ...
    if not POSITION_SIZING_FILE.exists():
        errors.append("Phase 5: trade/trade_sizing.py MISSING")
    else:
        out, code = sh(f'python3 -m py_compile "{POSITION_SIZING_FILE}" 2>&1', timeout=5)
        if code != 0:
            errors.append(f"Phase 5: trade_sizing.py syntax error — {out[:150]}")
        else:
            out, code = sh(
                f'python3 -c "import sys; sys.path.insert(0, \'{PROJECT_DIR}\'); from trade.trade_sizing import compute_position_size" 2>&1',
                timeout=5
            )
            if code != 0:
                errors.append(f"Phase 5: trade_sizing.py import FAILED — {out[:200]}")

    # 8b: trade_heat.py
    if not PORTFOLIO_HEAT_FILE.exists():
        errors.append("Phase 5: trade/trade_heat.py MISSING")
    else:
        out, code = sh(f'python3 -m py_compile "{PORTFOLIO_HEAT_FILE}" 2>&1', timeout=5)
        if code != 0:
            errors.append(f"Phase 5: trade_heat.py syntax error — {out[:150]}")
        else:
            out, code = sh(
                f'python3 -c "import sys; sys.path.insert(0, \'{PROJECT_DIR}\'); from trade.trade_heat import compute_portfolio_heat" 2>&1',
                timeout=5
            )
            if code != 0:
                errors.append(f"Phase 5: trade_heat.py import FAILED — {out[:200]}")
    ...
    if active_trades and len(active_trades) >= 2:
        try:
            out, code = sh(
                ...
                f'from trade.trade_sizing import COIN_SECTORS; '
...
def attempt_repair(error_msg, error_key):
    ...
    if "duplicate" in error_msg.lower() or "poll conflict" in error_msg.lower():
        ...
        if len(pids) > 3:
            ...
            for pid in pids_sorted[:-1]:
                sh(f"kill {pid} 2>/dev/null")
            return ("bot/bot_main.py", "Duplicate bot instances", f"Killed {len(pids_sorted)-1} old instances", True)

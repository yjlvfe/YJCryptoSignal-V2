...
# Both are read-only environment variables — never hardcoded.
BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN") or os.getenv("BOT_TOKEN") or ""
if not BOT_TOKEN:
    logger.error("❌ TELEGRAM_BOT_TOKEN not set in .env — bot cannot connect to Telegram. "
                  "Set TELEGRAM_BOT_TOKEN in /root/projects/YJCryptoSignal/.env")
    API_BASE = ""
else:
    API_BASE = f"https://api.telegram.org/bot{BOT_TOKEN}"

# TELEGRAM_CHAT_ID — chat for proactive alerts; falls back to OWNER_ID.
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID") or os.getenv("OWNER_ID") or ""
if not TELEGRAM_CHAT_ID:
    logger.warning("⚠️ TELEGRAM_CHAT_ID not set — proactive alerts will not be sent. "
                    "Set TELEGRAM_CHAT_ID or OWNER_ID in .env")

# Data directory - configurable via env, defaults to separate folder for YJCryptoSignal
...
def set_rate_limit(window_str: str, max_count: int):
    ...
    from bot.bot_ratelimit import parse_time_window  # lazy: avoid circular import
...
def get_role(user_id: int) -> str:
    ...
    from bot.bot_handlers import load_admins
    if user_id in load_admins():
        return "admin"
    # Check if has slot → premium
    from bot.bot_handlers import _read_slots
    slots = _read_slots()
    if user_id in slots.get("active", []):
        return "premium"
    from bot.bot_handlers import load_subscribers

...
from trade.trade_tracker import add_trade, check_trades, get_active_trades, format_trades_list, update_current_prices, load_trades, save_trades, generate_daily_report, cleanup_trades, MAX_TRADES, _calc_pnl
from sectors.sectors_categories import analyze_sectors, find_rotation_opportunity
from core.core_analyzer import Analyzer
from engine.engine_scanner import CryptoScanner
from data.data_fetcher import fetch_klines
from data.data_exchanges import check_availability
from report.report_sectors import format_sector_report
from report.report_telegram import format_signal_report, format_signal_report_arabic_simple, format_scan_summary
...
from trade.trade_userlists import (
    ...
    cleanup_closed_trade, get_all_user_entry_prices, get_user_entry_price,
    set_user_entry_price, set_user_target_count, get_user_target_count,
    has_tracking_data, is_tracking_active, mark_user_tracking_complete,
    mark_user_target_hit, get_active_trackers_for_symbol, cleanup_user_tracking,
    record_sale, get_user_sales, get_user_sales_summary, TRACKING_FILE
)
from engine.engine_multi_tf import analyze_mtf, scan_strength_matrix, format_strength_matrix
...
def run_scan(chat_id: int):
    ...
    try:
        ...
        from engine.engine_multi_tf import analyze_mtf
        ...
        ai_result = None
        try:
            from core.core_ai import analyze_coin, enrich_with_modules
    ...
    try:
        ...
        from data.data_fetcher import TOP_SYMBOLS
        from sectors.sectors_categories import get_all_sector_coins
...
def run_analyze(chat_id: int, symbol: str, full: bool = True, arabic: bool = False):
    ...
    try:
        ...
        if arabic:
            ...
            a = result.aggregated
            try:
                from core.core_smart_targets import enhance_signal_targets
            ...
            kronos_score = 0
            try:
                from engine.engine_kronos import compute_kronos
            ...
            layer_agreement = ""
            try:
                from engine.engine_layers import analyze_all_layers
...
def check_user_tracking_targets():
    ...
    try:
        from trade.trade_userlists import TRACKING_FILE
            ...
            for uid, info in users:
                ...
                for i in range(1, len(targets)):
                    if i not in targets_hit and i < len(targets) and cp >= targets[i]:
                        ...
                        from trade.trade_userlists import mark_user_target_hit, get_user_target_count, mark_user_tracking_complete, remove_from_user_list, unsubscribe_from_trade, get_user_entry_price, record_sale
                        ...
                        if is_last:
                            ...
                            from bot.bot_handlers import safe_send
                            safe_send(uid, (
                            ...
                            from bot.bot_handlers import send_msg_premium
                            reply_markup = {
                                "inline_keyboard": [
        ...
        try:
            ...
            if monitor_cycle == 1:  # first cycle only
                ...
                for line in result.stdout.split('\n'):
                    if 'bot/bot_main.py' not in line:
                        continue
                    ...
                    try:
                        ...
                        logger.debug(f"  duplicate-instance check failed for PID {pid}: {e}")
                if len(others) > 1:
                    logger.warning(f"⚠️ {len(others)} duplicate instances running (PIDs: {others}) — may cause Telegram 409 conflicts")
                elif others:
                    logger.info(f"📋 Found {len(others)} other instance(s) (PIDs: {others}) — systemd should manage lifecycle")
                ...
                if active_trades:
                    ...
                    if closed_symbols:
                        try:
                            ...
                            logger.warning(f"  ⚠️ failed reading trades_history.json for SL cooldown: {e}")
                            history = []
                    ...
                    try:
                        from trade.trade_heat import track_trade_lifecycle
                        for t in active_trades:
                            track_trade_lifecycle(t, is_closed=False)
                    except Exception as e:
                        logger.debug(f"  ⚠️ trade_heat lifecycle tracking failed: {e}")

...
logger = logging.getLogger("yjcrypto-weights")

WEIGHTS_FILE = Path("/root/.yjcryptosignal-bot/strategy_weights.json")
PERF_FILE = Path("/root/.yjcryptosignal-bot/strategy_performance.json")
SYMBOL_PERF_FILE = Path("/root/.yjcryptosignal-bot/symbol_performance.json")
...
def get_regime_adjusted_weights(regime_data: dict = None, base_weights: dict = None) -> dict:
    ...
    learning_mult = {}
    try:
        from learn.learn_adaptive import get_adjusted_weights
...
# 🆕 Expectancy tracking
EXPECTANCY_FILE = Path("/root/.yjcryptosignal-bot/strategy_expectancy.json")
